...
// TestPHPServerDirCanonicalRedirect covers the canonical trailing-slash
// directory-redirect route generated by php_server, which uses the
// MatchDirIndex matcher (a prefiltered equivalent of
// `{file {path}/index.php} + {not path */}`).
func TestPHPServerDirCanonicalRedirect(t *testing.T) {
	tester := caddytest.NewTester(t)
	initServer(t, tester, `
		{
			skip_install_trust
	...
	rootIndexBody := "I am by birth a Genevese (i not set)"

	// a regular PHP request is served normally (and, thanks to the
	// prefilter, without evaluating the directory-redirect file matcher)
	tester.AssertGetResponse("http://localhost:"+testPort+"/index.php", http.StatusOK, rootIndexBody)

	// a real directory containing index.php still gets the canonical 308
	// trailing-slash redirect...
	tester.AssertRedirect("http://localhost:"+testPort+"/dirindex", "http://localhost:"+testPort+"/dirindex/", http.StatusPermanentRedirect)

	// ...and its index file is served after the redirect
	tester.AssertGetResponse("http://localhost:"+testPort+"/dirindex/", http.StatusOK, "Hello from directory index.php")

	// Pathological case: a directory literally named "dirnamed.php".
	// The prefilter skips the canonical redirect for paths ending in a
	// split extension, so no 308 is issued; the request falls through the
	// try_files rewrite whose split_path logic truncates
	// "/dirnamed.php/index.php" to "/dirnamed.php" (a directory, not a
	// file), and thus falls back to the site index.
	// Previously a 308 to /dirnamed.php/ was issued first, but the
	// re-request then went through the exact same fallback, so the
	// response body is identical; only the redirect round-trip is gone.
	noRedirectClient := tester.Client
	oldCheckRedirect := noRedirectClient.CheckRedirect
	noRedirectClient.CheckRedirect = func(req *http.Request, via []*http.Request) error {
		return http.ErrUseLastResponse
	}
	resp, err := noRedirectClient.Get("http://localhost:" + testPort + "/dirnamed.php")
	require.NoError(t, err)
	body, err := io.ReadAll(resp.Body)
	require.NoError(t, err)
	require.NoError(t, resp.Body.Close())
	require.Equal(t, http.StatusOK, resp.StatusCode, "a directory named *.php must not trigger the canonical redirect")
	require.Equal(t, rootIndexBody, string(body), "a directory named *.php falls back to the site index")
	noRedirectClient.CheckRedirect = oldCheckRedirect

	// with a trailing slash the result is the same (the directory's own
	// index.php is shadowed by split_path handling either way)
	tester.AssertGetResponse("http://localhost:"+testPort+"/dirnamed.php/", http.StatusOK, rootIndexBody)
}
